...
import logging
import pandas as pd
from data_capture.models import bls_data, bls_pricing,bls_lcat,bls_occupation_lcat_mapping,bls_state,bls_state_city_mapping,bls_occs
import os

logging.basicConfig(level = logging.INFO, 
filename='loadDataLog.log', 
filemode='w', 
...
for index, row in excell.iterrows():
    ...
    if stateCode:
        ...
        for stateCode in stateCodeArr:
            ...
            if stateInstance:
                stateCityMappingInstance = bls_state_city_mapping.objects.filter(state = stateInstance[0],city = areaName)
    ...
    #mapping occs_code aaand lcat_title (this is only becase to extract the lcat title based on selected occuption)
    occupationLcatIns = bls_occupation_lcat_mapping.objects.filter(occupation_code = occupationCode , lcat = lcatTitleInstance)
    if not occupationLcatIns:
        occupationLcatIns = bls_occupation_lcat_mapping(occupation_code = occupationCode , lcat = lcatTitleInstance)
