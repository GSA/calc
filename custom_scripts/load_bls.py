...
import pandas as pd
from data_capture.models import bls_data,bls_lcat,bls_occupation_lcat_mapping,bls_state,bls_state_city_mapping
import os
...
xlpath = os.path.join(os.getcwd(),'custom_scripts/OASIS_LCAT_CROSSWALK__TITLES_TO_CATS_4.1.2015.xlsx')
sheetname = 'LCAT'
excell = pd.read_excel(xlpath,sheet_name=sheetname)
#making every column items lowercase
excell.columns = [alterColum(i) for i in excell.columns]
for index,row in excell.iterrows():
    ...
    if lcatInstance:
        if bls_occupation_lcat_mapping.objects.filter(occupation_code = occupationCode, lcat = lcatInstance):
...
for index,row in excell.iterrows():
    ...
    if isinstance(state_code, str):
        ...
        for state in state_code:
            ...
            if stateInstance:
                if not bls_state_city_mapping.objects.filter(state = stateInstance[0],city = cityName):
